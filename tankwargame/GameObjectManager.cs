...
namespace tankwargame
{
    class GameObjectManager
    {
        ...
        private static List<unbeweglich> wallList = new List<unbeweglich>();
        private static List<unbeweglich> steelList = new List<unbeweglich>();
        private static unbeweglich boss;
        ...
        {
            foreach (unbeweglich nm in wallList)
            {
                nm.Update();
            }
            foreach (unbeweglich nm in steelList)
            {
                nm.Update();
            }
        ...
        public static void DestroyWall(unbeweglich wall)
        {
        ...
        public static unbeweglich IsCollidedWall(Rectangle rt)
        {
            foreach (unbeweglich wall in wallList)
            {
        ...
        public static unbeweglich IsCollidedSteel(Rectangle rt)
        {
            foreach (unbeweglich wall in steelList)
            {
                if (wall.GetRectangle().IntersectsWith(rt))
                {
                    return wall;
        ...
        private static void CreateBoss(int x,int y,Image img)
        {
            int xPosition = x * 30;
            int yPosition = y * 30;
            boss = new unbeweglich(xPosition, yPosition, img);
        }

        private static void CreateWall(int x,int y,int count,Image img, List<unbeweglich> wallList)
        {
            int xPosition = x * 30;
            int yPosition = y * 30;
            for(int i = yPosition; i < yPosition + count * 30; i += 15)
            {
                ...
                unbeweglich wall1 = new unbeweglich(xPosition, i, img);
                unbeweglich wall2 = new unbeweglich(xPosition+15, i, img);
                wallList.Add(wall1);
                wallList.Add(wall2);
            }
        }
